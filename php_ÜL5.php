...
//Tüdrukute nimed
$nimed = array('Kendra', 'Mary-liis', 'Kertu', 'Kati', 'Liis', 'Annabel', 'Karina', 'Liisbet');
arsort($Tüdrukud);
...
<?php
echo '<xmp>'. $nimed[0] .'</xmp>';
echo '<xmp>'. $nimed[1] . '</xmp>';
echo '<xmp>'. $nimed[2] .'</xmp>';
?>

<h4>Viimane tüdruk listis</h4>
<?php
    echo end($nimed)
?>

<h4>Suvalise tüdruku nimi listist</h4>
<?php
$rand_keys = array_rand($nimed);
echo $nimed[$rand_keys] . "\n";

?>

<h4>Autode arv massiivis</h4>
<?php
...
<?php
$counts = array_count_values($autod);
echo '<xmp>'. $counts['Toyota'] .'</xmp>';
echo '<xmp>'. $counts['Audi'] .'</xmp>';
